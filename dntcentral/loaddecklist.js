...
for (var i = 0; i < cutOff; i++) {
    if (sortedList[1][i] == "-1") {
        ...
        firstHalf += `<tr><td>${sortedList[1][i]}&emsp;<a href=\"\" class=\"cellA\" onclick=\"return false;\" onmouseover=\"previewDeck(this.text)\">${sortedList[0][i]}</a></td></tr>`;
    }
...
var p3 = `</tbody></table></div></div></div><div class="col-6"><div class="previewDiv"><img src="https://gatherer.wizards.com/Handlers/Image.ashx?name=${sortedList[0][1]}&type=card" id="deckPreviewImg"></div></div></div>`;
...
for (var i = cutOff; i < sortedList[0].length; i++) {
    if (sortedList[1][i] == "-1") {
        ...
        console.log(sortedList[0][i]);
        console.log(i);
        secondHalf += `<tr><td>${sortedList[1][i]}&emsp;<a href=\"\" class=\"cellA\" onclick=\"return false;\" onmouseover=\"previewDeck(this.text)\">${sortedList[0][i]}</a></td></tr>`;
    }
}
...
newHTML += secondHalf;
newHTML += p3;
document.getElementById('autolist').innerHTML = newHTML;
